...
class frecuencia_dialog: DialogFragment() {
    override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
        return activity?.let {
            val alertBuilder = AlertDialog.Builder(it)
            var checkedIndex = ArrayList<Int>()

            alertBuilder.setTitle("Frecuencia")
            alertBuilder.setMultiChoiceItems(R.array.frecuencia_list, null,DialogInterface.OnMultiChoiceClickListener{
                _, index, checked ->
                if(checked){
